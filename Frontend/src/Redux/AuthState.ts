...
export function authReducer(currentState = new AuthState(), action: AuthAction): AuthState {
    ...
    switch (action.type) {
        ...
        case AuthActionType.Login:
            ...
            sessionStorage.setItem("token", newState.token); // Save token to storage for persisting token after refresh
            break;

        case AuthActionType.Logout:
            newState.token = null;
            newState.user = null;
            sessionStorage.removeItem("token"); // Remove token when logged out
            break;
